...
If a plan requires a schema migration: generate it with `payload migrate:create` and read the generated SQL yourself first. If it's purely additive, just run `payload migrate`. If it touches an existing column with data in it (localizing a field, narrowing a type, dropping/renaming a column), the migration must explicitly back-fill the data into the new shape before dropping anything — never assume Payload does this for you — and that one still needs Juan's explicit named approval before applying.
