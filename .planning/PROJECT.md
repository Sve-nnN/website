...
- **v2.0 (2026-07-31)**: auditoría externa `opengraph.to/u/juan-tech.com` encontró score 17/100 — el sitio nunca tuvo `og:image` en ninguna página, ni favicon/apple-touch-icon/theme-color/manifest/canonical consistente. El plugin SEO (`@payloadcms/plugin-seo`) ya expone un campo `meta.image` (upload) por doc, pero ningún `generateMetadata` del frontend lo lee hacia `openGraph.images` — el gap es de wiring, no de schema.
...
| OG image (v2.0) se genera vía transforms de Cloudinary (overlay de texto), no `next/og`/`ImageResponse` | Juan pidió explícitamente replicar el enfoque de `JuanPortfolio`; juan-payload usa la misma cuenta Cloudinary que ese sitio, que ya tiene el asset `og-scrim` y la fuente `Array-Bold.woff2` subidos — reusar es más barato que introducir un mecanismo de generación de imágenes nuevo | Pending (Phase 41) |
