...
- **Juan revisó `ripple` en vivo y prefirió `blob`** ("me gusta más blob que ripple"), a pesar de que `blob` — incluso con los valores del preset oficial de la librería — resulta en una textura mucho más sutil y "moody": con esta versión del paquete y la paleta de marca (tonos navy cercanos entre sí), los puntos calientes que orbita `blob` casi nunca cruzan el umbral de visibilidad completa, dejando una superficie casi negra con apenas un shimmer viviente de grano — investigado exhaustivamente (múltiples combinaciones de `intensity`/`softness`/`scale`/`offset`, confirmado con lecturas de píxeles reales vía `sharp`, no solo inspección visual) antes de concluir que es un comportamiento genuino del shader en esta versión, no un error de parámetros.
- **Decisión final: `shape="blob"`**, con valores ajustados a la paleta de marca (`softness=0.15`, `intensity=0.2`, `noise=0.35`, `scale=1.4`) — el resultado final es una lectura mucho más minimalista/oscura que la del `wave` original o la del `ripple` intermedio, y es lo que Juan prefirió al comparar ambas opciones en vivo.
