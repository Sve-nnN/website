...
### Update (2026-07-09, post-research addendum): documented custom-adapter reference found

Juan surfaced a working reference that changes the recommended default: [Sahitya Neupane, "How I Integrated Cloudinary with Payload CMS"](https://medium.com/@neupanesahitya1/how-i-integrated-cloudinary-with-payload-cms-so-you-dont-have-to-struggle-like-i-did-58ace37dd531) (published 2026-05-03), with full working code at [github.com/Sahitya1707/payload-cloudinary](https://github.com/Sahitya1707/payload-cloudinary) (`master` branch, last pushed 2026-05-03, still visible/citable as of 2026-07-09).

Verified directly against the repo's `package.json`: `payload@3.33.0`, `@payloadcms/plugin-cloud-storage@^3.36.1`, `@payloadcms/next@3.33.0`, `next@15.3.0`, `cloudinary@^2.6.0` — a real, version-pinned Payload 3.x example, not a 2.x-era post.

This is architecturally identical to the "fallback custom adapter" already described below (built on `@payloadcms/plugin-cloud-storage`'s `GeneratedAdapter` shape, implementing `handleUpload` via a Promise-wrapped `cloudinary.uploader.upload_stream`, `handleDelete`, `staticHandler` returning 501 since Cloudinary serves public URLs directly, and `generateFileURL`) — but it is no longer a hypothetical "you'd write ~100-150 lines following the S3/R2 template," it's a **concrete, working, copyable reference implementation** with a full example app around it (not just a snippet).

**This shifts the default recommendation.** A custom adapter built on `@payloadcms/plugin-cloud-storage` (official framework) using this repo as the starting template is now the **primary path**, ahead of the two community npm packages:

| Rank | Option | Why |
|---|---|---|
| 1 (new primary) | Custom adapter on `@payloadcms/plugin-cloud-storage`, adapted from `github.com/Sahitya1707/payload-cloudinary` | Zero third-party package risk (no dependency on a small solo/team npm package's continued maintenance) — you own the ~100-150 lines outright. Built on the *official* Payload framework, not a wrapper around a wrapper. Has a real reference implementation to start from instead of writing from the interface spec cold. Only gap: repo targets Payload 3.33, needs a compatibility check against 3.85 during the Phase 3 spike (breaking changes in `plugin-cloud-storage` between those versions are LOW probability per Payload's semver practice, but verify `handleUpload`/`GeneratedAdapter` signatures haven't shifted). |
| 2 | `@jhb.software/payload-cloudinary-plugin` | Keep as a comparison point during the spike if the custom adapter hits friction — team-backed, tightest version pin to current Payload. |
| 3 | `payload-storage-cloudinary` (nlvcodes) | Still viable but demoted — no longer the "leading candidate" now that a concrete custom-adapter path exists with less dependency risk. |
| — | `payload-cloudinary` (SyedMuzamilM) | Still **NO USAR** — unchanged, red flag stands. |

### Recommendation (superseded — see update above for current ranking)

~~Do NOT default to a hand-rolled custom adapter as the first move.~~ Superseded: the discovery of a working, version-verified reference implementation removes the main cost of the custom-adapter path (writing it blind), so it is now the recommended first move for the Phase 3 spike.

1. **Spike the custom adapter first**, using `github.com/Sahitya1707/payload-cloudinary` as the starting template: port `handleUpload`/`handleDelete`/`staticHandler`/`generateFileURL` into `src/lib/cloudinary-adapter.ts`, update for Payload 3.85 (from the repo's 3.33 baseline), wire against a real Cloudinary account, verify signed uploads, folder structure (`media/` prefix pattern from the reference), and that resulting URLs work in `next/image` with `remotePatterns` (mirroring the R2 pattern already in `apturio/website/next.config.mjs`).
2. **If the custom adapter hits real friction** (not just tedium — an actual blocker), fall back to spiking `@jhb.software/payload-cloudinary-plugin` (team-backed, tightest Payload version pin) as the second option, then `payload-storage-cloudinary` as the third.
3. Interface reference (for the custom adapter, Payload's `GeneratedAdapter` shape via `@payloadcms/plugin-cloud-storage`):
   - `name` — adapter identifier string
   - `handleUpload(args)` — receives the file buffer/stream + collection config, must upload to Cloudinary (via the `cloudinary` npm SDK's `uploader.upload_stream`, Promise-wrapped since the SDK is callback-based) and return
   - `handleDelete(args)` — deletes the corresponding Cloudinary asset by public ID
   - `generateURL(args)` / `generateFileURL` — builds the public Cloudinary delivery URL from the stored filename/public ID, so `next/image` gets direct Cloudinary URLs, not proxied through your Node server
   - `staticHandler(req, args)` — the reference implementation returns 501 "Not implemented" here since Cloudinary serves public URLs directly and this handler path isn't needed
   - Config: `disableLocalStorage: true`; envs `CLOUDINARY_NAME`/`CLOUDINARY_API_KEY`/`CLOUDINARY_API_SECRET` (align naming with whatever the final adapter code expects — the reference uses `CLOUDINARY_NAME`, official Cloudinary SDK docs commonly use `CLOUDINARY_CLOUD_NAME`, reconcile during implementation)

**Bottom line for roadmap:** Phase 3 spike now starts from a concrete reference instead of a blank interface — still time-boxed and still verified hands-on against a real Cloudinary account before committing, but materially lower risk than when this research was first written.
