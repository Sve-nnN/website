...
**How to avoid:**
- **Add `go` to the middleware matcher's negative lookahead:** `'/((?!api|admin|go|_next|_vercel|.*\\..*).*)'`. Verify by asserting the redirects-lookup route is not hit on a `/go/` request (log or test).
